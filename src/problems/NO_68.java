...
public class NO_68 {
	// 二叉搜索树
	public static TreeNode getLastCommonParent1(TreeNode root, TreeNode p, TreeNode q){
		if (root == null) {
			return root;
		}
		// 两个节点在右子树
		if (root.val < p.val && root.val < q.val) {
			return getLastCommonParent1(root.right, p, q);
		}
		// 两个节点都在左子树
		if (root.val > p.val && root.val > q.val) {
			return getLastCommonParent1(root.left, p, q);
		}
		Long res = 12L;
		res.intValue();
		// 节点在当前子树
